...
# ----------------------
  ...
  it '9. finds orders for a user' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    orders_of_user_3 = User.find(@user_3.id).orders
    # ------------------------------------------------------------
...
# ========================
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    items_for_user_3_third_order = Order.where(user: @user_3).group(:id).third.items.pluck(:name)
    # ------------------------------------------------------------
...
# ========================
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    # Solution goes here
    ordered_items = Item.joins(:order_items).order(:id).distinct.pluck(:name)
    # ---------------------------------------------------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:orders).distinct.select('users.name, count(orders.id) as total_order_count').group('orders.user_id').order("total_order_count")
    # ---------------------------------------------------------------
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(orders: :items).distinct.select("users.name, count(items.id) as total_item_count").group(:id).order(:name)
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User.joins(orders: :order_items).select("users.name as user_name, orders.id as order_id, orders.amount / count(order_items.id) as avg_item_cost").group("orders.id").order("name desc, avg_item_cost asc")
    # ---------------------------------------------------------------
